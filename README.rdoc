...
Stockpile is a simple key-value store connection manager, where each instance
can manage one or more connections to a key-value store. Currently, the only
implemented connection manager is for Redis.

Stockpile also provides an adapter mechanism that injects Stockpile
functionality into an application module.

== Features

* Stockpile manages key-value store connections. Each Stockpile always contains
  *one* connection, but additional client connections can be added as needed.

  * Stockpile works in a *wide* connection mode, where additional client
    connections are new instances of the client library.
  * Stockpile can work in a *narrow* connection mode, where additional client
    connections are the main Stockpile connection.

* Stockpile makes it easy to create a managed application-level connection
  manager, which can then be used for adapter methods to clean up code that
...
over the last few years when working with Redis. In general, I have seen a few
patterns that I consider to be suboptimal.

* Developers often use +REDIS+ or <tt>$redis</tt> to initialize and access
  their Redis instances. This could be fixed by using +Redis.current+, but that
  doesn’t help with exposing implementation details where they aren’t needed.
...
Stockpile addresses each of these, although very differently than I have
addressed them in the past.

=== Sample Rails Application

I will be adapting a sample Rails application to demonstrate how Stockpile is
to be used in Rails. A link to it will be provided when it is complete.
